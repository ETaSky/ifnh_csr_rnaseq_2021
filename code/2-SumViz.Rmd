...
# wikipathway file
wpgmtfile <- read.gmt("../data/reference/wikipathways-20210610-gmt-Mus_musculus.gmt")
...
for (ss in bodysites){
    # ss = "Ileum"
    de_res <- out[[ss]]$DESeq_res
    de_lfc <- de_res["log2FoldChange"] %>% as.data.frame()
    pv.out <- pathview(gene.data = de_lfc, pathway.id = "mmu", species = "mmu", out.suffix = "test/test")
}
```
